...
asmlinkage long interceptor(struct pt_regs reg){
	...
	if(check_pid_monitored(reg->ax, pid) == 1){
		log_message(pid, reg->ax, reg->bx, reg->cx, reg->dx, reg->si, reg->bi, reg-> bp);
	};
	return table->f(reg);
}
 ...
 *   Finished!! a) the syscall must be valid (not negative, not > NR_syscalls, and not MY_CUSTOM_SYSCALL itself)
 ...
 *      Finished!! For the first two commands, we must be root (see the current_uid() macro).
 ...
 *     Finished a) Cannot de-intercept a system call that has not been intercepted yet.
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	if (syscall < 0 || syscall > NR_syscalls || syscall == 0) {
		return -EINVAL;
	}
	...
	if (current_uid != 0) {
		...
		if (cmd == 3 || cmd == 4) {
			if (check_pid_from_list(pid,current->pid) == -EPERM || pid == 0) {
				return -EPERM;
			}
		}
	}
	...
	if (cmd == 3 && check_pid_monitored(syscall,pid) == 1) {
		return -EBUSY;
	}
